...
//Not working currently. due to some reasons
// CwcHome page get request
router.get('/cwcHome/:district/:cwcOfficialName', function (req, res) {
  res.render("cwcHome.ejs", {district: req.params.district, cwcOfficialName: req.params.cwcOfficialName});
});
...
// This is to create attendance for a specific CCi[JUST FRO TESTING]
...
// this an API for getting info of attendance.
//usinfg this api we show data of attendance for the required date through AJAX request.
//This dont render any ejs page.
router.get('/getDetails/:cciId/find/:changeDate', function (req, res) {
  ...
  Cci.find({cci_id: req.params.cciId,  "attendance.date": chosenDate },{attendance:{$elemMatch: {date: chosenDate}}},function(err, foundCci){
    if(err){
      ...
      if(foundCci.length>0){//If attendance is uploaded for that date
...
//this route works when attendnce list button is clicked on the page of  cci Details.
// ROUTE FOR CONNECTING ATTENDANCE PAGE MADE BY TANISHA 
//this route show ejs page attendanceList for current date on default
router.get('/newAttendance/:district/:cwcOfficialName/:cciId/:cciname', function (req, res) {
  let cciid = req.params.cciId;
  var today = new Date();
  console.log(today);
